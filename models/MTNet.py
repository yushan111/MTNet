...
class MTNet:
    def __init__(self, config, scope='MTNet'):
        ...
        with tf.variable_scope(scope, reuse=False):
            # X: long-term time series historical data
            # shape=[num of long-term time series, time step, feature_num]
            X = tf.placeholder(tf.float32,
                               shape=[None, self.config.n, self.config.T, self.config.D])
            # Q: Short term history data, shape=[time step, feature_num]
            Q = tf.placeholder(tf.float32, shape=[None, self.config.T, self.config.D])
            # Y: output. shape = [value_num]
            ...
            u = self.__encoder(tf.reshape(Q, shape=[-1, 1, self.config.T, self.config.D]), 1,
                               scope='in')
            ...
            with tf.variable_scope('AutoRegression'):
                if self.config.highway_window > 0:
                    highway_ws = tf.get_variable('highway_ws',
                                                 shape=[self.config.highway_window * self.config.D,
                                                        self.config.K],
                                                 dtype=tf.float32,
                                                 initializer=tf.truncated_normal_initializer(
                                                     stddev=0.1))
                    highway_b = tf.get_variable('highway_b', shape=[self.config.K],
                                                dtype=tf.float32,
    ...
    def __encoder(self, origin_input_x, n, strides=[1, 1, 1, 1], padding='VALID',
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('CNN', reuse=tf.AUTO_REUSE):
                w_conv1 = tf.get_variable('w_conv1', shape=[self.config.W, self.config.D, 1,
                                                            self.config.en_conv_hidden_size],
                                          dtype=tf.float32,
                                          initializer=tf.truncated_normal_initializer(stddev=0.1))
                b_conv1 = tf.get_variable('b_conv1', shape=[self.config.en_conv_hidden_size],
                                          dtype=tf.float32,
                                          initializer=tf.constant_initializer(0.1))
